...
void workThread(int threadId) {
    for (int i = 0; i < 30; ++i) {
        Logger::getInstance().log(LogLevel::INFO, "thread=%d, count=%d", threadId, i);
        Logger::getInstance().log(LogLevel::DEBUG, "thread=%d debug info", threadId);
    }
}
...
int main() {
    ...
    logger.setLogFile("./app.log");           // 传入 "" 就关闭文件输出，只打印控制台
